...
# Configuration
# ──────────────────────────────
# If etl is ran independently, this returns a logger with no handler
# If etl is imported and ran, this returns the logger created from root
...
def _timestamped_path(base_path: Path) -> Path:
    """
...
def _configure_logging(log_path: Path) -> None:
    ...
    """
    # Checking if root logger already exists or not
    if logging.getLogger().hasHandlers():
        return
    # Initializes handler for logger
    logging.basicConfig(
        level=logging.DEBUG,
    ...
    logger.debug("Logging to %s", log_path.resolve())
...
def load_json(file_path: Path) -> pd.DataFrame:
    ...
    orient: str | None          # Expected JSON arrangement ("records", "split", ...)
    typ: str                    # "frame" = DataFrame, "series" = Series
    dtype: bool | dict | None   # Force dtypes by column name
    lines: bool | None          # True if each line is a separate JSON object (NDJSON)
    precise_float: bool         # Whether to use high precision for decoding strings to double values
    convert_dates: bool         # Attempt to parse ISO8601 dates
    encoding: str               # Character encoding
    encoding_errors: str        # How encoding errors are handled ("strict", "ignore", "replace", ...)
    nrows: int | None           # Read only first N rows
    """
    ...
    logger.info("JSON loaded → rows=%d, cols=%d", *df.shape)
    return df
...
def load_excel(
    ...
    *,
    ...
    if isinstance(df, dict):
        logger.info("Excel loaded: multiple sheets (%d)", len(df))
    else:
        logger.info("Excel loaded → rows=%d, cols=%d", *df.shape)
    return df
...
def transform_dataframe(
    ...
    *,
    ...
    for new_col, func in config.items():
        logger.debug("Computing column '%s' with %s cores", new_col, n_jobs)
        ...
        logger.debug("Finished '%s'", new_col)
...
def main(*, input_file: Path) -> None:
    ...
    input_file : Path
        The file to process.
    """
    logger.debug("Starting process on %s", input_file)
...
if __name__ == "__main__":
    # Set Up
    args = _parse_args()
    full_log_path = _timestamped_path(args.log)
    _configure_logging(full_log_path)
